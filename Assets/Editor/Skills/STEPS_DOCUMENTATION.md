...
- **ScaleMultiplier** (float) — общий множитель размера VFX (1 = без изменения).
- **ScaleX**, **ScaleY** (float) — дополнительные множители по горизонтали и вертикали (1 = без изменения). Итоговый масштаб по X = `(1 + (AoeScale − 1)) × ScaleMultiplier × ScaleX`. По Y вертикальный вклад AoE вдвое слабее: `(1 + (AoeScale − 1) × 0.5) × ScaleMultiplier × ScaleY`.
...
**Назначение:** Ищет цели в зоне удара и наносит урон по снапшоту статов (DamageCalculator). Зону можно задать вручную (радиус, offset) или привязать к позиции/размеру степа SpawnVFX через **Source step index**. Без source это квадрат со стороной `2 × Radius`; с source — прямоугольник по текущему кадру VFX. Вертикальный AoE растёт вдвое слабее горизонтального.

**Параметры (оверрайды):**
- **Radius** (float) — базовый радиус, если SourceStepIndex = −1 (по умолчанию 1.5).
- **SizeX**, **SizeY** (float) — множители хитбокса по осям (1 = без изменения). Работают и с source, и без него.
- **OffsetX**, **OffsetY** (float) — смещение центра от игрока (если SourceStepIndex = −1).
- **SourceStepIndex** (int) — индекс степа в рецепте (обычно Spawn VFX), откуда брать **позицию и визуальный размер**. Тот степ должен выполниться раньше в том же кадре или раньше по времени (ставьте Spawn VFX выше в списке и тот же Trigger at %).
...
**Параметры (оверрайды):**
- **SizeX**, **SizeY** (float) — размер бокса. Если SourceStepIndex = −1, это мировой размер; вертикальный AoE растёт вдвое слабее горизонтального. Если source задан, это множители от визуального размера VFX.
